...
export async function GET(request: NextRequest) {
  ...
  const envStatus = {
    ...
    emailConfig: {
      ...
      fromEmailValue: process.env.EMAIL_FROM
        ? `${process.env.EMAIL_FROM.substring(0, 3)}...`
        : "not set",
      replyToConfigured: !!process.env.EMAIL_REPLY_TO,
      replyToValue: process.env.EMAIL_REPLY_TO
        ? `${process.env.EMAIL_REPLY_TO.substring(0, 3)}...`
        : "not set",
  ...
  // Return the status
  return new NextResponse(JSON.stringify(envStatus, null, 2), {
    status: 200,
