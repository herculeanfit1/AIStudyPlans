...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch email usage statistics" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to reset email usage counters" }, { status: 500 });
  }
}
