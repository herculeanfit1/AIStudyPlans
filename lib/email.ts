...
import {
  checkEmailQuota,
  getEmailUsageWarnings,
  recordEmailFailure,
  recordEmailSent,
} from "./email-monitor";
import {
  getPasswordResetTemplate,
  getWaitlistAdminNotificationTemplate,
...
export async function sendEmail({
  ...
  if (!quotaCheck.allowed) {
    ...
    const error = Object.assign(new Error(`Email sending blocked: ${quotaCheck.reason}`), {
      quotaExceeded: true,
  ...
  try {
    ...
    const isOptimalTime =
      (hour >= 9 && hour <= 11) || (hour >= 4 && hour <= 7) || (hour >= 16 && hour <= 19);
    // Check if we're sending on optimal days (Tues-Thurs)
    const isOptimalDay = dayOfWeek >= 2 && dayOfWeek <= 4;

    // eslint-disable-next-line no-console
    console.log(
      `📧 Sending email to ${to} on ${dayNames[dayOfWeek]} at ${hour}:${now.getMinutes().toString().padStart(2, "0")} - ${isOptimalDay ? "Optimal day" : "Non-optimal day"}, ${isOptimalTime ? "Optimal time" : "Non-optimal time"}`,
    );
...
export async function sendWaitlistConfirmationEmail(email: string) {
  ...
  // eslint-disable-next-line no-console
  console.log(
    "📊 Sending immediate welcome email - 74% of subscribers expect this and open rates average 80%",
  );
...
export async function sendFeedbackCampaignEmail(user: WaitlistUser) {
  ...
  // eslint-disable-next-line no-console
  console.log(
    `📧 Sending ${emailNames[nextPosition]} to ${user.email} (sequence position ${sequencePosition}->${nextPosition})`,
  );
  ...
  if (user.last_email_sent_at) {
    ...
    const now = new Date();
    const daysSinceLastEmail = Math.round(
      (now.getTime() - lastEmailDate.getTime()) / (1000 * 60 * 60 * 24),
    );
    // eslint-disable-next-line no-console
    console.log(
      `⏱️ It has been ${daysSinceLastEmail} days since their last email (research recommends 3-7 days for first email, 7-14 days for subsequent emails)`,
    );
  }

  // Get the appropriate template
  const { html, text, subject } = getFeedbackEmailTemplate(nextPosition, {
    appUrl,
    user,
    feedbackFormUrl,
  });
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log(
      `✅ Successfully sent ${emailNames[nextPosition]} to ${user.email} (ID: ${result?.messageId})`,
    );
