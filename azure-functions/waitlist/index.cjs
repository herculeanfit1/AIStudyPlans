...
module.exports = async (context, req) => {
  ...
  try {
    ...
    safeLog(
      context,
      `Environment config: URL=${appUrl}, FROM=${fromEmail}, REPLY_TO=${replyToEmail}`,
    );
    ...
    const emailToUse =
      process.env.NODE_ENV === "production" && !isTestEmail ? email : "delivered@resend.dev";

    safeLog(
      context,
      `Email decision: using ${emailToUse} for delivery (isTestEmail: ${isTestEmail})`,
    );
    ...
    safeLog(
      context,
      `Confirmation email sent successfully. ID: ${confirmationResult.messageId || "n/a"}`,
    );
    ...
    safeLog(
      context,
      `Admin notification email sent successfully. ID: ${adminNotificationResult.messageId || "n/a"}`,
    );
      ...
      body: JSON.stringify({
        ...
        note:
          process.env.NODE_ENV !== "production"
            ? "In development mode, emails are sent to delivered@resend.dev"
            : undefined,
      }),
    };
  } catch (error) {
...
async function sendWaitlistAdminNotification(name, email) {
  ...
  const adminEmailToUse =
    process.env.NODE_ENV === "production" ? adminEmail : "delivered@resend.dev";
