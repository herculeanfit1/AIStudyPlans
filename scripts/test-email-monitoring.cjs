...
try {
  ...
  console.log(
    `${colors.red}❌ Error testing email monitoring functions: ${error.message}${colors.reset}`,
  );
}
...
try {
  ...
  console.log(
    `${colors.red}❌ Error checking email service integration: ${error.message}${colors.reset}`,
  );
}
...
console.log(
  `1. ${colors.blue}Test in production:${colors.reset} Deploy and monitor actual email usage`,
);
console.log(
  `2. ${colors.blue}Set limits:${colors.reset} Configure EMAIL_DAILY_LIMIT and EMAIL_MONTHLY_LIMIT in production`,
);
console.log(
  `3. ${colors.blue}Monitor dashboard:${colors.reset} Access /api/admin/email-usage when authenticated`,
);
console.log(
  `4. ${colors.blue}Test quota enforcement:${colors.reset} Try sending emails when limits are reached`,
);
