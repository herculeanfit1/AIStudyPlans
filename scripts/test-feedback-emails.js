...
const { getFeedbackEmailTemplate } = require("../lib/feedback-email-templates");
...
async function main() {
  ...
  try {
    ...
    const { html, text, subject } = getFeedbackEmailTemplate(position, {
      appUrl,
      user,
      feedbackFormUrl,
    });
    ...
    if (testEmail === "delivered@resend.dev") {
      ...
      console.log(
        "   This email will appear in your Resend dashboard, but will not be delivered to an actual inbox.",
      );
