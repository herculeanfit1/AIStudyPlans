...
async function removeDenyAssignments(resourceId) {
  ...
  try {
    ...
    const denyAssignments = JSON.parse(
      executeAzCommand(`az resourcelock list --scope ${resourceId} -o json`),
    );

    if (denyAssignments && denyAssignments.length > 0) {
      console.log(
        `${colors.yellow}Found ${denyAssignments.length} locks/deny assignments.${colors.reset}`,
      );
...
async function main() {
  ...
  console.log(
    `${colors.cyan}This script will help you delete Azure resources that may have deny assignments.${colors.reset}\n`,
  );
  ...
  const resources = JSON.parse(
    executeAzCommand(
      "az resource list --query \"[?contains(name, 'aistudyplans')].{id:id, name:name, resourceGroup:resourceGroup, type:type}\" -o json",
    ),
  );
  ...
  resources.forEach((resource, index) => {
    console.log(
      `${index + 1}. ${resource.name} (${resource.type}) - Resource Group: ${resource.resourceGroup}`,
    );
  });
  ...
  const resourceGroups = JSON.parse(
    executeAzCommand(
      "az group list --query \"[?contains(name, 'aistudyplans')].{name:name}\" -o json",
    ),
  );
  ...
  if (deleteChoice === "1") {
    ...
    const confirmDelete = await askQuestion(
      `${colors.red}${colors.bold}Are you sure you want to delete this resource? (yes/no): ${colors.reset}`,
    );
    ...
    if (deleteResult !== null) {
      ...
      console.log(
        `Run: az resource move --destination-group "AIStudyPlans-RG1" --ids ${resource.id}`,
      );
    }
    ...
    const confirmDelete = await askQuestion(
      `${colors.red}${colors.bold}Are you sure you want to delete this entire resource group? (yes/no): ${colors.reset}`,
    );
    ...
    const groupResources = JSON.parse(
      executeAzCommand(`az resource list --resource-group ${resourceGroup.name} -o json`),
    );

    if (groupResources && groupResources.length > 0) {
      console.log(
        `${colors.yellow}Found ${groupResources.length} resources in the group.${colors.reset}`,
      );
    ...
    const deleteResult = executeAzCommand(
      `az group delete --name ${resourceGroup.name} --yes --verbose`,
    );
