...
if (process.env.CI === "true") {
  console.log(
    `${colors.yellow}Detected CI environment, skipping interactive checks${colors.reset}`,
  );
  process.exit(0);
}
...
try {
  ...
  try {
    ...
    console.log(
      `${colors.green}✓ No security vulnerabilities found in production dependencies${colors.reset}`,
    );
  } catch (error) {
    console.warn(`${colors.yellow}⚠ Security vulnerabilities found in dependencies${colors.reset}`);
    console.warn(
      `${colors.yellow}  Run 'npm audit' for details and 'npm audit fix' to attempt fixing${colors.reset}`,
    );
  }
