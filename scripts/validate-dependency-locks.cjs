...
class DependencyLockValidator {
    ...
    try {
      ...
      for (const [name, version] of Object.entries(allDeps)) {
        ...
        if (shrinkwrapEntry && shrinkwrapEntry.version !== version.replace(/[\^~]/, "")) {
          this.warnings.push(
            `Version mismatch for ${name}: package.json(${version}) vs shrinkwrap(${shrinkwrapEntry.version})`,
          );
        }
      }
    } catch (error) {
  ...
  async checkSecurityVulnerabilities() {
    ...
    console.log(
      colors.yellow("🔍 Basic security check (run npm run lock:audit for comprehensive scan)"),
    );
  }
  ...
  async checkDependencyDrift() {
    ...
    console.log(
      colors.yellow("📊 Basic drift check complete (run npm run lock:check for detailed analysis)"),
    );
  }
