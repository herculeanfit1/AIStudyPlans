...
async function main() {
  ...
  console.log(
    `${colors.cyan}This script will help you create a new Application Insights resource in Azure.${colors.reset}\n`,
  );
  ...
  if (
    isNaN(resourceGroupIndex) ||
    resourceGroupIndex < 0 ||
    resourceGroupIndex >= resourceGroups.length
  ) {
  ...
  const locations = JSON.parse(
    executeAzCommand('az account list-locations --query "[].name" -o json'),
  );
  ...
  if (locationInput.toLowerCase() === "full") {
    ...
    if (
      isNaN(fullLocationIndex) ||
      fullLocationIndex < 0 ||
      fullLocationIndex >= locations.length
    ) {
      console.error(`${colors.red}Invalid selection. Please run the script again.${colors.reset}`);
      process.exit(1);
    }
    ...
    if (!locations.includes(location)) {
      console.log(
        `${colors.yellow}Warning: '${location}' is not in the standard location list. Continuing anyway.${colors.reset}`,
      );
    }
  }
  ...
  try {
    ...
    const createOutput = executeAzCommand(
    ...
    const connectionString = executeAzCommand(
      `az monitor app-insights component show --app ${appInsightsName} --resource-group ${resourceGroup} --query connectionString -o tsv`,
    );
    ...
    console.log(
      `2. Run the migration script: node scripts/migrate-app-insights.js OLD_CONNECTION_STRING "${connectionString}"`,
    );
    console.log(`3. Test your application to ensure monitoring is working correctly`);
    console.log(
      `4. Once confirmed working, you can safely delete the old Application Insights resource`,
    );
  } catch (error) {
    console.error(
