...
console.log(
  `${colors.cyan}${colors.bold}=== Authentication Configuration Verification ===${colors.reset}`,
);
console.log();
...
for (const [key, description] of Object.entries(requiredEnvVars)) {
  if (process.env[key]) {
    ...
    console.log(
      `${colors.red}✗ ${key}${colors.reset}: ${description} ${colors.red}(MISSING)${colors.reset}`,
    );
    envErrors++;
  }
}
...
try {
  ...
  console.log(
    `${colors.red}✗ Error checking NextAuth.js configuration: ${error.message}${colors.reset}`,
  );
  nextAuthErrors++;
}
...
console.log(
  `${colors.blue}${colors.bold}4. Checking for conflicting authentication methods${colors.reset}`,
);
...
for (const file of conflictingFiles) {
  ...
  if (fs.existsSync(filePath)) {
    console.log(
      `${colors.red}✗ Found conflicting file: ${file.path}${colors.reset} - ${file.description}`,
    );
    conflictsFound++;
  }
}
...
if (totalErrors === 0) {
  console.log(
    `${colors.green}Authentication configuration looks good! No issues detected.${colors.reset}`,
  );
} else {
  console.log(
    `${colors.yellow}Found ${totalErrors} issues with authentication configuration:${colors.reset}`,
  );
