...
if (resendApiKey) {
    ...
    try {
      ...
      if (error) {
        ...
        if (domains.data && domains.data.length > 0) {
          console.log(
            `${colors.green}✅ Domains configured: ${domains.data.map((d) => d.name).join(", ")}${colors.reset}`,
          );
        } else {
  ...
  console.log(
    `${colors.yellow}ℹ️  Set up the required environment variables to enable email functionality${colors.reset}`,
  );
  process.exit(1);
}
